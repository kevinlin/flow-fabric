...
export class Inbox {
  ...
  handleWait(info: UserTaskWaitInfo): void {
    ...
    this.events.append({ instanceId: info.instanceId, type: 'usertask.created', elementId: info.nodeId });
  ...
  async submit(taskId: number, vars: Record<string, unknown>): Promise<void> {
    ...
    this.events.append({ instanceId: task.instanceId, type: 'usertask.submitted', elementId: task.nodeId });
    if (task.taskExecutionId !== null) {
      this.store.finishTaskExecution(task.taskExecutionId, { status: 'completed', output: vars });
      this.events.taskExecution(task.taskExecutionId);
    }
  }
}
