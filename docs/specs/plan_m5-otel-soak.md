...
- 2026-07-20 — Implemented the code half (Tasks 1–6, 8, 9) on branch `m5-otel-soak`. Status:
  - **Done (code + unit tests, all green):** Task 1 deterministic ids, Task 2 task spans + task metrics, Task 3 instance root span + event-log span events + run metrics + incident counter, Task 4 `NOOP_TELEMETRY` + config-gated `initTelemetry`, Task 5 `InstanceStore` seams, Task 6 daemon wiring + exports + `.env.example`, Task 8 `soak/report.ts` + CLI, Task 9 launchd unit + runbook.
  - Server suite: 118 passed / 5 skipped (was 105); +13 new (`telemetry.test.ts` 8, `telemetry-store.test.ts` 5, `soak-report.test.ts` 6). `pnpm build && pnpm test` green across shared/server/web.
  - **API-drift finding (behavior, not types):** OTel SDK (`sdk-trace-base@2.9`) treats `startTime: 0` as falsy and substitutes `Date.now()`; a non-zero epoch is honored literally. Only bit the synthetic root-span test (`startedAt: 0`); production passes `row.createdAt` (a real epoch), so the path is unaffected. Test uses `1_000` and documents why. No code workaround needed. The version floors held — no `SimpleSpanProcessor({exporter})` / `TracerProvider` rename encountered.
  - **Live smoke (not the full gate):** built daemon boots `OTel export disabled` (env unset) and `OTel export enabled` (`OTEL_EXPORTER_OTLP_ENDPOINT` set), and SIGINT prints `stopping engines, flushing telemetry` then exits cleanly. Ran on a scratch data dir + free port; did not touch the 24/7 daemon. `soak-report.ts` smoke: `0 instance(s), 0 timer cycle(s), 0 SILENT-STALL(s)`, exit 0. `run-daemon.sh` passes `sh -n`; the sed-substituted plist passes `plutil -lint`.
  - **Operator-gated, NOT executed (need Docker / a real API key + workspace / 7 days):** Task 7 (M5.1 Jaeger trace gate), Task 10 (M5.2 first real run + cost review), Task 11 (M5.3 7-day soak), Task 12 (M5.4 cost/priming decision + doc-state flip). `docs/specs/index.md` and `CLAUDE.md` are deliberately left as-is — M5 stays "not accepted" until the G1 soak passes; flipping them to Done now would misrepresent unverified acceptance.
