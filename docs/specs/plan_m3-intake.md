...
- 2026-07-19 — **Linter readability + auto-fix suggestions.** FF001–FF006 messages now name nodes by their BPMN label instead of the raw Signavio sid (`lint.ts` builds an `id→name` map for the graph rules FF004/FF005; a `TYPE_LABEL` map turns `bpmn:ServiceTask` into "agent (service) task"). `LintFinding` (shared) gained `nodeName?` and `suggestion?`. The `finding()` helper takes an options object; grill-fixable rules — FF002–FF004, FF006, and the generic-`bpmn:Task` case of FF001 — attach a ready-to-send grill instruction as `suggestion`; FF005 orphans and structural FF001 (parallel gateway, timeCycle, message flow, malformed events) omit it. Covered by three new `linter.test.ts` cases (label-not-id, suggestion presence, suggestion absence); server suite 20 → 23.