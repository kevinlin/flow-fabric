...
- 2026-07-19 — **Refine layout rework + canvas zoom controls.** `RefinePage` went from a side-by-side `diagram | chat` grid to a stacked flex column (`.content > .refine { max-width: none }` overrides the 1160px section cap): a full-width top row holds the `BpmnCanvas` (`.refine-top`, `flex: 3`), and `.refine-bottom` (`flex: 2`) splits into two columns — `.refine-lint` (lint report + Save footer) and `.refine-chat` (grill chat). `.bpmn-canvas` dropped its fixed `height: 480px; flex: 1` for a responsive `width: 100%; aspect-ratio: 16 / 9; min-height: 360px`; inside `.refine-top` it fills the band by height (`aspect-ratio: auto; height: 100%`) so the diagram is the dominant block without blowing out viewport height. `BpmnCanvas` gained overlaid zoom in / out / reset controls (bpmn-js `canvas.zoom(scale, 'auto')`, `zoom('fit-viewport')` for reset) over a dedicated `.bpmn-viewport` child, so the viewer owns its own DOM node. The 16:9 canvas + zoom controls also apply on Instance detail (shared component). Covered by new `bpmn-canvas.test.tsx` (3 tests, mocked NavigatedViewer); web suite 19 → 22, `tsc` + `vite build` green.
- 2026-07-19 — **Lint panel readability + one-click auto-fix.** `LintPanel` now shows each finding's `nodeName` (BPMN label) in the `<em>`, with the raw id kept as a `title` tooltip, instead of printing the opaque sid. Grill-fixable findings render an "Ask grill to fix" button (`onSuggest`/`busy` props) that sends the finding's server-supplied `suggestion` to the grill agent immediately. `RefinePage.send()` was refactored to take the text explicitly (`send(text = draft)`) so a button click doesn't fire on the stale `draft` closure; the panel is wired as `<LintPanel report={lint} busy={busy} onSuggest={send} />`. New `.fix-btn` CSS. Covered by four new `lint-panel.test.tsx` cases (label-over-id, button click → `onSuggest`, no button without suggestion, disabled while busy); web suite 22 → 26.
- 2026-07-19 — **Markdown rendering for grill chat replies.** Agent messages in the Refine grill chat render as Markdown (GFM: tables, strikethrough, task lists) instead of plain text; user messages stay literal (`RefinePage` chat map branches on `m.who` — agent → `<Markdown>`, you → raw text). New `Markdown` component (`packages/web/src/components/Markdown.tsx`) wraps `react-markdown` + `remark-gfm`: react-markdown injects no raw HTML and strips dangerous URLs by default, so untrusted agent output needs no separate sanitizer; a custom `a` renderer opens links with `target="_blank" rel="noreferrer"`. Added `.chat-msg .markdown` CSS — collapsed leading/trailing block margins so the bubble hugs its content, `code`/`pre` surfaces (`--canvas` bg + hairline border) that read against the stone agent bubble, plus table/list/blockquote spacing. Deps: `react-markdown@^10`, `remark-gfm@^4` (web only, +97 transitive — first break from the zero-new-lib pattern, chosen over a hand-rolled parser for markdown correctness + XSS safety). Covered by `markdown.test.tsx` (4 cases: emphasis + inline code, lists + fenced code block, GFM table, link `target` + `javascript:` strip); web suite 26 → 30, `tsc` + `vite build` green.
- 2026-07-19 — **Delete undeployed definitions.** Added `DefinitionStore.delete(id)` with an instance-guard: queries the shared SQLite `instances` table (same DB) and throws `DefinitionInUseError` (409) if any instance references the definition; otherwise deletes all versions + the definition row in a single transaction. Handles standalone DBs without an `instances` table (test isolation) via `sqlite_master` check. New `DELETE /api/definitions/:id` route (204 success, 404 not found, 409 in use). `DefinitionsPage` gained a "Delete" button per definition card (`.def-actions` flex row beside the Refine link, `.btn-danger` destructive styling) with a `window.confirm` guard; 409 errors surface in the page error banner. `api.deleteDefinition(id)` added to the web client. Design spec §8 updated with the DELETE route; §9 updated to mention delete. Covered by 3 new store unit tests (success, not-found, in-use guard) + 3 new API tests (204, 404, 409); server suite 105 → 111, web suite 30 unchanged, `pnpm build && pnpm test` green.
- 2026-07-20 — **Composition root (`createDaemon`) + `stopAll()` shutdown hardening.** New `packages/server/src/compose.ts`: `createDaemon(opts): Daemon` wires the full graph (InstanceStore, DefinitionStore, EngineHost, Inbox, GrillHost, LogRing, `buildApi`) that `daemon.ts` and ~10 test files each hand-rolled (the `let inbox!` two-phase bind ×10, the `{ notify: async () => {} }` stub ×9). Defaults are inert — no-op notifier, stub-runner dry-run path, NOOP telemetry, fail-fast grill queryFn — so a bare `createDaemon({ dataDir })` can never notify, call the Claude SDK, or export telemetry; `daemon.ts` slimmed to the one entrypoint injecting production adapters (`AgentRunner`/`CodeRunner`, `MacNotifier`, `initTelemetry()`, new `sdkQueryFn` export from `grill/session.ts`, webRoot) and now runs `daemon.close()` on SIGTERM. `Daemon.close()` owns teardown order (stopAll → app.close → telemetry.shutdown → store/definitions close, idempotent). Code review caught two real bugs, both fixed test-first: (1) GrillSession's implicit fallback to the live SDK when no queryFn is injected — the RED test spent 17 s in a real SDK call; compose now defaults to a thrower, prod injects `sdkQueryFn` explicitly; (2) `EngineHost.stopAll()` missed engines still inside `engine.execute()`, so an immediate `close()` shut the DB under a live run (probe: 20/20 leaked never-settling completions — the M4 events-vocab unhandled-rejection finding, reintroduced through a side door). `EngineHost` now `track()`s every start/resume chain synchronously, `stopAll()` sets a `stopping` flag and awaits `Promise.allSettled` of in-flight runs, and late-registering engines stop themselves. Migrated test files: api, api-reads, metrics, definitions, grill-api, events-vocab, dry-run-e2e, user-tasks, static, timeline — each `afterEach` collapses to `await d.close()`. New `compose.test.ts` (5 tests: inert dry run over HTTP, close() on a waiting engine + idempotence, grill fail-fast, close-right-after-start settles, telemetry shutdown after stopAll). Server suite 129 → 134 (1 pre-existing skip), `tsc` green; live daemon probe: healthz + SPA + `/api/logs` served, SIGTERM → graceful close → exit 0. Design spec §3 (module table + amendment), §6.2 (stopAll semantics), §11 (composition test row) updated; CLAUDE.md daemon/entrypoint sections updated; root `CONTEXT.md` created (Daemon, Workspace terms).
- 2026-07-20 — **Events module carve-out (architecture review §1).** The design spec (§3) named an `events` module — single write path, SSE fan-out, OTel emission — that M4 had folded into `InstanceStore`, leaving the persistence DAO doubling as the event bus and the telemetry emitter (a terminal `setStatus` re-SELECTed the whole row plus the full event list to assemble a span; the `EventEmitter` + fan-out lived in the store; `EngineHost`, which owns the run, held no telemetry reference). New `packages/server/src/events/events.ts` (`Events`) extracts them behind `append(event)` + `subscribe(listener, filter?)` plus three telemetry drivers — `instanceEnded`/`taskExecution`/`incidentRaised` — that own span-payload assembly and the terminal-transition dedup `Set`. `InstanceStore` dropped to pure persistence: `EventEmitter` + `setMaxListeners` + `onEvent` gone, `{telemetry?}` option gone, `Telemetry`/`EventEmitter` imports gone; `appendEvent` → `insertEvent` (INSERT, returns seq, no fan-out), `setStatus` a plain UPDATE. Adapters repointed at the one seam: the SSE route (`api/server.ts`) went `store.onEvent` → `deps.events.subscribe(listener, { instanceId })` (the query filter is now the subscribe filter); OTel (`Telemetry`) is driven only by `Events`, gated on `telemetry.enabled` so a NOOP run pays none of the assembly SELECTs (replaces the old "deliberate asymmetry" in `compose.ts` that withheld telemetry from the store). `EngineHost` now holds `events` (`EngineHostOptions.events`, defaulting to an inert `Events` over the same store so standalone host tests need no wiring) and calls `events.instanceEnded` on every terminal transition (run() completed/terminated/aborted/error + `abort()` — the dedup Set makes the double-call a no-op and preserves the abort-with-no-live-engine span); dispatch/failure/inbox forward `taskExecution`/`incidentRaised` after the store write. `compose.ts` constructs the one shared `Events` and injects it into host, inbox, and API, and exposes it on `Daemon`. Behavior-preserving structural refactor (telemetry stays NOOP by default) that de-risks M5's OTel wiring — after it, M5.1 is a one-file change. Test migration: `telemetry-store.test.ts` (5 tests, faked telemetry inside the store) folded into new `events.test.ts` (11 tests: append/subscribe fan-out + filter + unsubscribe, `instanceEnded` dedup + event-log assembly + unknown-row skip, `taskExecution`/`incidentRaised` forwarding, disabled-telemetry gate — all asserting the injected `Telemetry` spy through `Events`, never inside the store); `events-vocab.test.ts` / `api.test.ts` SSE / `compose.test.ts` teardown-order all stayed green unchanged. Server suite 134 → 140 (net +6; 5 skipped), `tsc` clean (strict, NodeNext), `pnpm build` green across the workspace (web/shared untouched). The resume SIGKILL timing test flakes only under the full parallel run (documented real-timer flakiness) — green in isolation. Design spec §3 (events-module amendment), §8 (SSE served by `events.subscribe`), §10 (Events is the single OTel driver), §11 (events test row) updated.